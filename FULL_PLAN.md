...
# Stability work (recent)

- Normalized intra-package imports across `raymarching_benchmark` (use package-relative imports) to ensure the package can be imported reliably.
- Added a lightweight smoke test (`tests/test_smoke.py`) covering `Vec3`, `Camera` and `StandardSphereTracing` to prevent regressions.
- Fixed `strategies/segment_tracing.py` import issues and ensured all strategies use package-safe imports.

These changes are intended to make the codebase importable and runnable with standard tooling (pytest, linters).
├── metrics/
│   ├── __init__.py
...
The following minimal artifacts were added to make the project runnable/testable quickly:

- `raymarching_benchmark/main.py` — small CLI and `run_once()` integration helper
